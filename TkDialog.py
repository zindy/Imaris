...
class TkDialog(tk.Tk):
    ...
    def bake(self, has_cancel=False, has_live=None, has_preview=None, has_ok=True):
        ...
        for i in range(n_widgets):
            ...
            #Now add the widget
            widget = self._controls[self._controlnames[i]]
            widget_parent = self._parents[self._controlnames[i]]
            tickcontrol = self._tickcontrols[self._controlnames[i]]
            ...
            if type(widget) == list:
                ...
                if widget_parent == None:
                    for j in range(nw):
                        w = widget[j]
                        if j == nw-1 and tickcontrol is None:
                            span = 2
                        else:
                            span = 1
            ...
            if tickcontrol is not None and type(tickcontrol)!=int:
                ...
                tickcontrol.grid_configure(sticky=(tk.W))
